...
def get_rpm_header(nvr, tries=0):
    ...
    raise ValueError(f"No rpm headers found in koji for {nvr!r}")
...
def sorted_updates(updates):
    ...
    for package in sorted(builds.keys()):
        if len(builds[package]) > 1:
            ...
            build = next(iter(builds[package]))
    ...
    if not (len(set(sync) & set(async_)) == 0 and len(set(sync) | set(async_)) == len(updates)):
        ...
        raise Exception("ERROR! SYNC+ASYNC != UPDATES! sorted_updates failed")  # noqa: TRY002
    return sync, async_
...
def waiverdb_api_post(waiverdb_api_url, data):
    ...
    return call_api(
        ...
        headers={
            ...
            "Authorization": f"Bearer {config.get('waiverdb.access_token')}",
        },
    )
...
def _get_build_repository_and_digest(build):
    ...
    digest_pull_spec = next(spec for spec in pull_specs if "@" in spec)

    base, digest = digest_pull_spec.split("@", 1)
    _server, repository = base.split("/", 1)
...
def copy_container(build, destination_registry=None, destination_tag=None):
    ...
    for dr in destination_registry_list:
        destination_url = _container_image_url(
            dr, repository, tag=make_valid_container_tag(destination_tag)
        )
...
def pyfile_to_module(filename: str, modname: str, silent: bool = False) -> types.ModuleType | bool:
    ...
    try:
        ...
        if silent and e.errno in (errno.ENOENT, errno.EISDIR):
            return False
        e.strerror = f"Unable to load file ({e.strerror})"
        raise
    return d
