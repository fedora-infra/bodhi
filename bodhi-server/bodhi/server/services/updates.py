...
@update_edit.get(
    accept="text/html",
    renderer="new_update.html",
    error_handler=bodhi.server.services.errors.html_handler,
    permission="edit",
    validators=(validate_acls,),
)
...
def set_request(request):
    ...
    if update.locked:
        request.errors.add("body", "request", "Can't change request on a locked update")
        return

    if update.status == UpdateStatus.stable and action == UpdateRequest.testing:
        request.errors.add(
            "body", "request", "Pushing back to testing a stable update is not allowed"
        )
        return
...
@updates_rss.get(
    schema=bodhi.server.schemas.ListUpdateSchema(),
    renderer="rss",
    error_handler=bodhi.server.services.errors.html_handler,
...
def query_updates(request):
    ...
    if like is not None:
        ...
        query = query.filter(Build.nvr.like(f"%{like}%"))

    search = data.get("search")
    if search is not None:
        query = query.join(Update.builds)
        query = query.filter(or_(Build.nvr.ilike(f"%{search}%"), Update.alias.ilike(f"%{search}%")))
    ...
    count_query = (
        ...
        .statement.with_only_columns(func.count(distinct(Update.id)))
        .order_by(None)
    )
    ...
    if request.accept.accept_html():
        return_values.update(
            gating_statuses=sorted(bodhi.server.models.TestGatingStatus.values()),
...
def new_update(request):
    ...
    try:
        ...
        for nvr in build_nvrs:
            _name, _version, release = request.buildinfo[nvr]["nvr"]
        ...
        if data.get("edited"):
            log.info(f"Editing update: {data['edited']}")

            data["release"] = next(iter(releases))
                ...
                caveats.append(
                    {
                        ...
                        f"into {len(releases)}, one for each release.",
                    }
                )
            updates = []
            for release in releases:
                ...
                log.info(f"Creating new update: {_data['builds']!r}")
                result, _caveats = Update.new(request, _data)
                log.debug(f"{result.alias} update created")
        ...
        log.warning(str(e))
        request.errors.add("body", "builds", str(e))
        return
    except Exception as e:  # noqa: BLE001
        log.exception("Failed to create update")
        request.errors.add("body", "builds", f"Unable to create update.  {e}")
        return
    ...
    for update in updates:
        try:
            ...
            caveats.append(
                {
                    "name": "update",
                    "description": f"Problem obsoleting older updates: {e}",
                }
            )
...
@update_waive_test_results.post(
    schema=bodhi.server.schemas.WaiveTestResultsSchema(),
    validators=(colander_body_validator, validate_update_id, validate_qa_acls),
    permission="edit",
    renderer="json",
    error_handler=bodhi.server.services.errors.json_handler,
)
...
@update_get_test_results.get(
    schema=bodhi.server.schemas.GetTestResultsSchema(),
    validators=(validate_update_id),
    renderer="json",
    error_handler=bodhi.server.services.errors.json_handler,
)
...
@update_trigger_tests.post(
    schema=bodhi.server.schemas.TriggerTestsSchema(),
    validators=(colander_body_validator, validate_update_id, validate_qa_acls),
    permission="edit",
    renderer="json",
    error_handler=bodhi.server.services.errors.json_handler,
        ...
        if update.content_type == ContentType.rpm:
            message = update_schemas.UpdateReadyForTestingV3.from_dict(
                message=update._build_group_test_message(
                    agent=request.identity.name, retrigger=True
                )
            )
