...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Print user data in human readable format. The script defaults to JSON output.",
)
...
def get_user_data(username, human_readable):
    ...
    sar_data[user.name]["comments"] = [
        {
            "karma": c.karma,
            "karma_critpath": c.karma_critpath,
            "text": c.text,
            "timestamp": c.timestamp.strftime("%Y-%m-%d %H:%M:%S"),
            "update_alias": c.update.alias,
    ...
    sar_data[user.name]["updates"] = [
        {
            ...
            "stable_karma": u.stable_karma,
            "unstable_karma": u.unstable_karma,
            "require_bugs": u.require_bugs,
            "require_testcases": u.require_testcases,
            "notes": u.notes,
            "type": str(u.type),
            "severity": str(u.severity),
            "suggest": str(u.suggest),
            "close_bugs": u.close_bugs,
            "alias": u.alias,
            "builds": [b.nvr for b in u.builds],
            "release_name": u.release.name,
            "bugs": [b.bug_id for b in u.bugs],
            "user": u.user.name,
