...
def upgrade():
    ...
    op.execute(
        "INSERT INTO build_testcase_table "
...
def downgrade():
    ...
    op.add_column(
        "testcases", sa.Column("package_id", sa.INTEGER(), autoincrement=False, nullable=True)
    )
    op.create_foreign_key(
        "testcases_package_id_fkey", "testcases", "packages", ["package_id"], ["id"]
    )
    op.drop_constraint("uq_testcases_name", "testcases", type_="unique")

    # Bring back data to the new package relationship
    op.execute(
        "UPDATE testcases AS t "
