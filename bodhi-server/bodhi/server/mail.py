...
""",
        "fields": lambda agent, x: {
            "package": x.title,
            "email": agent,
            "release": f"{x.release.long_name} {x.status}",
            "updatestr": str(x),
        },
    },
    "edited": {
    ...
    for build in update.builds:
        ...
        if update.bugs:
            ...
            for bug in update.bugs:
                ...
                if update.type is UpdateType.security and parent and not bug.parent:
                    log.debug("Skipping tracker bug %s", bug.bug_id)
                    continue
                title = (
                ...
                info["references"] += f"  [ {i} ] Bug #{bug.bug_id}{title}\n        {bug.url}\n"
                i += 1
...
def send(
    ...
    if msg_type != "buildroot_override":
        ...
        initial_message_id = "<bodhi-update-{}-{}-{}@{}>".format(
            update.id,
            update.user.name,
            update.release.name,
