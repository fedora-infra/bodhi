...
class EnumSymbol:
    ...
    def __ge__(self, other):
        """
        Return True if self.value is greater or equal than other.value.

        Args:
            other (EnumSymbol): The other EnumSymbol we are being compared to.
        Returns:
            bool: True if self.value is ge than other.value, False otherwise.
        """
        return self.value >= other.value
...
class DeclEnumType(SchemaType, TypeDecorator):
    ...
    def __init__(self, enum):
        ...
        self.impl = Enum(
            *enum.values(),
            name=f"ck{re.sub('([A-Z])', lambda m: '_' + m.group(1).lower(), enum.__name__)}",
...
class BodhiBase:
    ...
    def find_polymorphic_child(cls, identity):
        ...
        if not isinstance(identity, EnumSymbol):
            raise TypeError(f"{identity!r} is not an instance of EnumSymbol")

        if "polymorphic_on" not in getattr(cls, "__mapper_args__", {}):
            raise KeyError(f"{cls!r} is not a polymorphic model.")
        ...
        raise NameError(f"Found no child of {cls!r} with identity {identity!r}")
...
class Release(Base):
    ...
    def get_tags(cls):
        ...
        for release in cls.query.filter(
            ...
            for status, taglist in data.items():
                tag = getattr(release, f"{status}_tag")
                taglist.append(tag)
    ...
    def from_tags(cls, tags, session):
        ...
        """
        _tag_types, tag_rels = cls.get_tags()
        for tag in tags:
...
class Package(Base):
    ...
    def get_pkg_committers_from_pagure(self):
        ...
        * The second list contains FAS group names that have commit access.
        ...
        package_pagure_url = "{}/api/0/{}/{}?expand_group=1".format(
    ...
    def __str__(self):
        ...
        for state, updates in states.items():
            if updates:
                x += f"\n {state.title()} Updates ({len(updates)})\n"
                for update in updates:
                    x += f"    o {update.get_title()}\n"
        del states
        return x
        ...
        for tag in self.get_tags(koji):
            ...
            if tag == release.testing_tag:
                if not from_side_tag:
                    log.info("Moving %s from %s to %s", self.nvr, tag, release.candidate_tag)
...
class Update(Base):
    ...
    def edit(cls, request, data):
        ...
        comment = f"{request.identity.name} edited this update."
        if new_builds:
            comment += "\n\nNew build(s):\n"
    ...
    def test_gating_passed(self) -> bool:
        ...
        """
        return self.test_gating_status in (None, TestGatingStatus.ignored, TestGatingStatus.passed)
    ...
    def obsolete_older_updates(self, db):
        ...
        for build in self.builds:
            for oldBuild in (
                ...
                # obsolete them
                if (
                    len(oldBuild.update.builds) != len(self.builds)
                    and oldBuild.update.user.name != self.user.name
                ):
                    caveats.append(
                        {
                            "name": "update",
                            "description": (
                                "Please be aware that there is another update "
                                f"in flight owned by {oldBuild.update.user.name}, "
                                f"containing {oldBuild.nvr}. Are you coordinating "
                                "with them?"
                            ),
                        }
                    )
                ...
                if obsoletable:
                    ...
                    link = f"[{oldBuild.nvr}]({oldBuild.update.abs_url()})"
    ...
    def get_tags(self):
        ...
        """
        return list(set(chain.from_iterable(b.get_tags() for b in self.builds)))
    ...
    def set_request(self, db, action, username):
        ...
        if action is self.request:
            log.debug("%s has already been submitted to %s", self.alias, self.request.description)
            return
        ...
        if action is UpdateRequest.unpush:
            ...
            notifications.publish(
                update_schemas.UpdateRequestUnpushV1.from_dict({"update": self, "agent": username})
            )
            ...
            notifications.publish(
                update_schemas.UpdateRequestObsoleteV1.from_dict(
        ...
        # set the status to unpushed
            ...
            notifications.publish(
                update_schemas.UpdateRequestRevokeV1.from_dict({"update": self, "agent": username})
            )
            return
        ...
        comment_text = (
            f"This update has been submitted for {action.description} by {username}. {notes}"
        )
        ...
        notifications.publish(
            action_message_map[action].from_dict({"update": self, "agent": username})
        )

        if action == UpdateRequest.testing and config["test_gating.required"]:
    ...
    def send_update_notice(self):
        ...
        if self.status is UpdateStatus.stable:
            mailinglist = config.get(f"{release_name}_announce_list")
        elif self.status is UpdateStatus.testing:
            mailinglist = config.get(f"{release_name}_test_announce_list")
            ...
            for subject, body in mail.get_template(self, self.release.mail_template):
                ...
                notifications.publish(
                    errata_schemas.ErrataPublishV1.from_dict(
                        {"subject": subject, "body": body, "update": self}
                    )
                )
        else:
...
"""
    ...
    def update_bugs(self, bug_ids, session):
        ...
        for bug in to_remove:
            ...
            if (
                len(bug.updates) == 0
                and not session.query(BugKarma).filter_by(bug_id=bug.bug_id).count()
            ):
    ...
    def comment(
        ...
        if self.user.name == author and karma != 0:
            karma = 0
    ...
    def unpush(self, db):
        ...
        if self.status is not UpdateStatus.testing:
            raise BodhiException(f"Can't unpush a {self.status.description} update")
    ...
    def revoke(self):
        ...
        if self.status not in [
            ...
            raise BodhiException(
                ...
                f"update, not one that is {self.status.description}"
            )
    ...
    def untag(self, db, preserve_override: bool = False):
        ...
        koji = buildsys.get_session()
        _tag_types, tag_rels = Release.get_tags()
    ...
    def obsolete(self, db, newer=None):
        ...
        if newer:
            self.comment(
                db,
                f"This update has been obsoleted by [{newer.nvr}]({newer.update.abs_url()}).",
                author="bodhi",
            )
        else:
            self.comment(db, "This update has been obsoleted.", author="bodhi")
    ...
    def last_modified(self):
        ...
        if not possibilities:  # Should be un-possible.
            raise ValueError(f"Update has no timestamps set: {self!r}")
        ...
        for build in self.builds:
            ...
            for test in build.testcases:
                if test.name not in test_names:
                    ...
                    tests.add(test)
        return sorted(tests, key=lambda testcase: testcase.name)
...
class Compose(Base):
    ...
    def from_updates(cls, updates):
        ...
        for update in updates:
            ...
            # dict.
            key = f"{update.release.name}-{update.request.value}"
            if key not in work:
                work[key] = cls(
    ...
    def __lt__(self, other):
        ...
        if other.security and not self.security:
            return False
        return self.request == UpdateRequest.stable and other.request != UpdateRequest.stable
...
class Comment(Base):
    ...
    def __str__(self) -> str:
        ...
        karma = "0"
        if self.karma != 0:
            karma = f"{self.karma:+d}"
        return f"{self.user.name} - {self.timestamp} (karma: {karma})\n{self.text}"
...
class Bug(Base):
    ...
    def close_bug(self, update: Update) -> None:
        ...
        versions = {b.nvr_name: b.nvr for b in update.builds}
...
class BuildrootOverride(Base):
    ...
    def new(cls, request: "pyramid.request", **data) -> "BuildrootOverride":
        ...
        if build.override is not None:
            request.errors.add("body", "nvr", f"{build.nvr} is already in a override")
            return
    ...
    def enable(self) -> None:
        ...
        notifications.publish(override_schemas.BuildrootOverrideTagV1.from_dict({"override": self}))
