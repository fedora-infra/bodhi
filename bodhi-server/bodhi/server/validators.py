...
def cache_tags(request, build):
    ...
    try:
        ...
        if len(tags) == 0:
            request.errors.add(
                "body", "builds", f"Cannot find any tags associated with build: {build}"
            )
    except koji.GenericError:
        request.errors.add("body", "builds", f"Invalid koji build: {build}")
...
def cache_nvrs(request, build):
    ...
    if not kbinfo:
        ...
        raise ValueError(f"Build {build} did not exist")
...
def validate_build_nvrs(request, **kwargs):
    ...
    for build in request.validated.get("builds") or []:  # cope with builds being None
        try:
            ...
            request.validated["builds"] = []
            request.errors.add("body", "builds", f"Build does not exist: {build}")
            return
        except koji.GenericError:
            log.exception("Error retrieving koji build for %s", build)
            request.validated["builds"] = []
            request.errors.add("body", "builds", f"Koji error getting build: {build}")
            return
...
def validate_builds(request, **kwargs):
    ...
    if edited:
        ...
        if not up:
            request.errors.add("body", "builds", f"Cannot find update to edit: {edited}")
            return

        # Allow admins to edit stable updates
        user_groups = {group.name for group in user.groups}
        admin_groups = set(config["admin_packager_groups"])
        if not (user_groups & admin_groups) and up.status is UpdateStatus.stable:
...
def validate_build_tags(request, **kwargs):
    ...
    for build in request.validated.get("builds") or []:
        ...
        if edited and build_rel is not release:
            request.errors.add(
                "body",
                "builds",
                f"Cannot add a {build_rel.name} build to an {release.name} update",
            )
            return
        ...
        if from_tag and from_tag not in tags:
            # The build MUST be tagged in the side tag
            request.errors.add("body", "builds", f"Invalid build: {build} not tagged in {from_tag}")
...
def validate_tags(request, **kwargs):
    ...
    for tag_type in tag_types:
        tag_name = request.validated.get(f"{tag_type}_tag")
        ...
        try:
            ...
            request.validated[f"{tag_type}_tag"] = tag_name

        except Exception:  # noqa: BLE001
            request.errors.add("body", f"{tag_type}_tag", f"Invalid tag: {tag_name}")
    ...
    for build in builds:
        ...
        if "builds" in request.validated:
            ...
            try:
                ...
                error = f"Unable to infer content_type.  '{e}'"
                log.exception(error)
                request.errors.add("body", "builds", error)
...
def validate_update(request, **kwargs):
    ...
    if update:
        ...
        request.errors.add("url", "update", f"Invalid update specified: {idx}")
...
def _validate_override_build(request, nvr, db):
    ...
    if build is not None:
        ...
        try:
            ...
            request.errors.add("body", "nvr", f"Couldn't determine koji tags for {nvr}, '{e}'")
            return
...
def validate_expiration_date(request, **kwargs):
    ...
    if expiration_date > limit:
        request.errors.add(
            "body", "expiration_date", f"Expiration date may not be longer than {days}"
        )
        return
        ...
        for other_build in other_builds:
            ...
            if rpm.labelCompare(other_build.evr, build.evr) > 0:
                ...
                request.errors.add(
                    ...
                    "update",
                    (
                        f"Cannot submit {build.package.name} {build.evr} "
                        f"to {target.description} since it is older than {other_build.evr}"
                    ),
                )
                request.errors.status = HTTPBadRequest.code
                return
