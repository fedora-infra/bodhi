...
class TestGetTemplate(BasePyTestCase):
    ...
    def test_skip_tracker_bug(self, debug):
        ...
        debug.assert_called_once_with("Skipping tracker bug %s", 12345)
...
"""
    ...
    def test_read_template_path_error(self, error):
        ...
        error.assert_called_once_with("Path does not exist: %s", template_path)
    ...
    def test_read_template_io_error(self, error):
        ...
        error.assert_has_calls(
            [
                mock.call("Unable to read template file: %s", template_path),
                mock.call("IO Error[%s]: %s", None, None),
            ]
        )
...
class Test_SendMail:
    ...
    def test_recipients_refused(self, SMTP, warning):
        ...
        warning.assert_called_once_with(
            '"recipient refused" for %r, %r', "lana@spies.com", smtp.sendmail.side_effect
        )
