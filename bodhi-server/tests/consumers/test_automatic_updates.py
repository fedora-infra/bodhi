...
class TestAutomaticUpdateHandler(base.BasePyTestCase):
    ...
    def test_consume(self, critpath, caplog):
        ...
        with mock.patch("bodhi.server.models.util.greenwave_api_post") as mock_greenwave:
            ...
            if critpath is True:
                ...
                with (
                    ...
                    mock.patch("bodhi.server.util.read_critpath_json") as fakejson,
                ):
        ...
        update = (
            ...
            .join(Build)
            .filter(Update.builds.any(Build.nvr == self.sample_nvr))
            .first()
        )
    ...
    def test_changelog(self, mock_generate_changelog, changelog):
        ...
        if changelog:
            assert (
                update.notes
                == f"""Automatic update for colord-1.3.4-1.fc26.
...
```"""
    ...
    def test_changelog_too_long(self, mock_generate_changelog):
        ...
        assert (
            update.notes
            == """Automatic update for colord-1.3.4-1.fc26.
...
```"""
    ...
    def test_existing_pending_update(self, caplog):
        ...
        assert (
            f"Build, active update for {self.sample_nvr} exists already, skipping."
            in caplog.messages
        )
    ...
    def test_obsolete_testing_update(self, caplog):
        ...
        new_update = (
            self.db.query(Update).join(Build).filter(Update.builds.any(Build.nvr == nvr)).first()
        )
    ...
    def test_problem_obsoleting_older_update(self, caplog):
        ...
        msg.body["build_id"] = 442563
        with mock.patch(
            "bodhi.server.models.Update.obsolete_older_updates",
    ...
    def test_too_long_nvr(self, caplog):
        ...
        msg = deepcopy(self.sample_message)
        msg.body["version"] = (
            "2.5.4^20251110gitf355928-2.20251110gitf355928.s20260106gitbb5417c"
    ...
    def test_unknown_koji_build(self, caplog):
        ...
        self.handler(msg)
        assert any(
            r.levelno == logging.DEBUG and r.getMessage().startswith("Can't find Koji build for ")
            for r in caplog.records
        )
    ...
    def test_missing_user(self, caplog):
        ...
        assert f"Creating bodhi user for '{expected_username}'." in caplog.messages
    ...
    def test_existing_user(self, caplog):
        ...
        assert f"Creating bodhi user for '{expected_username}'." not in caplog.messages
    ...
    def test_ignored_tag(self, caplog):
        ...
        assert any(
            x.startswith(f"Ignoring build being tagged into '{bogus_tag}'") for x in caplog.messages
        )
    ...
    def test_user_in_blacklist(self, caplog):
        ...
        with mock_sends():
            self.handler(self.sample_message)
        assert (
            f"{self.sample_nvr} owned by lmacken who is listed in "
