...
class TestRelease(ModelTest):
    ...
    def test_all_releases(self, debug):
        ...
        state = ReleaseState.from_string(next(iter(releases.keys())))
...
class TestContainerPackage(ModelTest):
    ...
    attrs = {"name": "docker-distribution"}
...
class TestBuild(ModelTest):
    ...
    klass = model.Build
    attrs = {"nvr": "TurboGears-1.0.8-3.fc11"}
...
class TestUpdateEdit(BasePyTestCase):
    ...
    def test_gating_required_false(self):
        ...
        request.identity.name = "tester"
        with (
            mock_sends(update_schemas.UpdateEditV2),
            mock.patch("bodhi.server.models.util.greenwave_api_post") as mock_greenwave,
        ):
            greenwave_response = {
...
class TestUpdate(ModelTest):
    ...
    def test_modify_bugs_stable_no_close(self, comment, close):
        ...
        assert (
            all("pushed to the Fedora 11 stable repository" in c[1][1] for c in comment.mock_calls)
            is True
        )
    ...
    def test_comment_emails_other_commenters(self):
        ...
        with (
            mock.patch("bodhi.server.mail.smtplib.SMTP") as SMTP,
            mock.patch.dict(
    ...
    def test_can_waive_test_results_of_an_update(self, mock_waiverdb, mock_greenwave):
        ...
        decision = {
            ...
            "unsatisfied_requirements": [
                {
                    "subject_identifier": "{update.builds[0].nvr}",
                    "subject_type": "koji_build",
                    "testcase": "dist.depcheck",
                    "scenario": "kde",
                    "type": "test-result-failed",
...
class TestBuildrootOverride(ModelTest):
    ...
    attrs = {
        "notes": "This is needed to build foobar",
