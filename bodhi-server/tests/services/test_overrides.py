...
class TestOverridesService(base.BasePyTestCase):
    ...
    def test_list_overrides_rss(self):
        res = self.app.get("/rss/overrides/", headers={"accept": "application/atom+xml"})
    ...
    def test_list_overrides_by_invalid_expired(self):
        res = self.app.get("/overrides/", {"expired": "lalala"}, status=400)
    ...
    def test_list_overrides_by_invalid_packages(self):
        res = self.app.get("/overrides/", {"packages": "flash-player"}, status=400)
    ...
    def test_list_overrides_by_invalid_releases(self):
        res = self.app.get("/overrides/", {"releases": "F42"}, status=400)
    ...
    def test_list_overrides_by_multiple_usernames(self):
        ...
        override = BuildrootOverride(
            build=build, submitter=another_user, notes="Crazy! 😱", expiration_date=expiration_date
        )
...
blah blah blah"""
    ...
    def test_create_override_multiple_nvr(self):
        ...
        with fml_testing.mock_sends(
            override_schemas.BuildrootOverrideTagV1, override_schemas.BuildrootOverrideTagV1
        ):
            res = self.app.post("/overrides/", data)
    ...
    def test_create_override_for_newer_build(self):
        ...
        build = RpmBuild(
            nvr="bodhi-2.0-2.fc17", package=old_build.package, release=old_build.release
        )
        self.db.add(build)
        self.db.flush()
    ...
    def test_cannot_edit_override_build(self):
        ...
        res = self.app.get(f"/overrides/{old_nvr}", headers={"Accept": "application/json"})
        o = res.json_body["override"]
        expiration_date = o["expiration_date"]
        old_build_id = o["build_id"]

        build = RpmBuild(
            nvr="bodhi-2.0-2.fc17",
            release=release,
    ...
    @mock.patch(
        "bodhi.server.services.overrides.BuildrootOverride.edit",
    ...
    def test_create_override_with_missing_pkg(self):
        ...
        with fml_testing.mock_sends(override_schemas.BuildrootOverrideTagV1):
            res = self.app.post("/overrides/", data, headers={"Accept": "application/json"})
...
class TestOverridesWebViews(base.BasePyTestCase):
    def test_override_view_not_loggedin(self):
        ...
        resp = app.get("/overrides/bodhi-2.0-1.fc17", status=200, headers={"Accept": "text/html"})
    ...
    def test_override_view_loggedin(self):
        ...
        """
        resp = self.app.get(
            "/overrides/bodhi-2.0-1.fc17", status=200, headers={"Accept": "text/html"}
        )
    ...
    def test_override_new_not_loggedin(self):
        ...
        app = webtest.TestApp(main({}, session=self.db, **anonymous_settings))
        resp = app.get("/overrides/new", status=403, headers={"Accept": "text/html"})
    ...
    def test_override_new_loggedin(self):
        ...
        """
        resp = self.app.get("/overrides/new", status=200, headers={"Accept": "text/html"})
    ...
    def test_overrides_list(self):
        ...
        in the test data.
        """
        resp = self.app.get("/overrides/", status=200, headers={"Accept": "text/html"})
    ...
    def test_override_expired_date(self, get):
        ...
        assert (
            "<span class='col-xs-auto pe-2 ms-auto text-danger'><small>"
