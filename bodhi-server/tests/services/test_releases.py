...
class TestReleasesService(base.BasePyTestCase):
    ...
    def test_anonymous_cant_edit_release(self):
        ...
        res = app.get(f"/releases/{name}", status=200)
        r = res.json_body
        r["edited"] = name
        r["state"] = "current"
    ...
    def test_list_releases_by_ids_plural(self):
        ...
        assert {r["name"] for r in res.json_body["releases"]} == {
    ...
    def test_list_releases_by_nonexistent_update(self):
        ...
        assert res.json_body["errors"][0]["description"] == "Invalid updates specified: carbunkle"
    ...
    def test_list_releases_by_nonexistent_package(self):
        ...
        assert res.json_body["errors"][0]["description"] == "Invalid packages specified: carbunkle"
    ...
    def test_new_release_invalid_tags(self):
        ...
        for error in res.json_body["errors"]:
            assert error["description"] == f"Invalid tag: {attrs[error['name']]}"

    def test_edit_release(self):
        name = "F22"

        res = self.app.get(f"/releases/{name}", status=200)
        r = res.json_body
    ...
    def test_change_release_state_to_archived(self):
        ...
        python_nose = self.db.query(Build).filter_by(nvr="python-nose-1.3.7-11.fc17").one().update
        assert python_nose.status == UpdateStatus.stable
        # Expect update status not changed
        python_paste_deploy = (
            self.db.query(Build).filter_by(nvr="python-paste-deploy-1.5.2-8.fc17").one().update
        )
        assert python_paste_deploy.status == UpdateStatus.obsolete
        # Expect update status not changed
        firefox = self.db.query(Build).filter_by(nvr="firefox-61.0.2-3.fc17").one().update
        assert firefox.status == UpdateStatus.unpushed
        # Expect update status changed to 'obsolete'
        bodhi_update = self.db.query(Build).filter_by(nvr="bodhi-2.0-1.fc17").one().update
        assert bodhi_update.status == UpdateStatus.obsolete
        assert bodhi_update.request is None
        # Check for the comment
        expected_comment = "This update is marked obsolete because the F17 release is archived."
        assert bodhi_update.comments[-1].text == expected_comment
        # Expect update status not changed
        python_test_update = (
            self.db.query(Build).filter_by(nvr="python-test-update.fc22").one().update
        )
...
class TestReleasesHTML(base.BasePyTestCase):
    ...
    def test_release_counts(self):
        ...
        assert (
            ...
            'until freeze is lifted">frozen</span>'
        ) in res
