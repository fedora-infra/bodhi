...
class TestCommentsService(base.BasePyTestCase):
    def setup_method(self, method):
        ...
        self.db.add(update)
        build = RpmBuild(
            nvr=up2, update=update, package=RpmPackage.query.filter_by(name="bodhi").one()
        )
        self.db.add(build)
        self.db.flush()

    def make_comment(self, nvr="bodhi-2.0-1.fc17", text="Test", karma=0, **kwargs):
    ...
    def test_invalid_karma(self):
        res = self.app.post_json("/comments/", self.make_comment(karma=-2), status=400)
    ...
    @mock.patch(
        "bodhi.server.services.comments.Update.comment", side_effect=OSError("IOError. oops!")
    )
    ...
    def test_get_single_comment_page(self):
        res = self.app.get("/comments/1", headers={"accept": "text/html"})
        assert "text/html" in res.headers["Content-Type"]
        assert "libravatar.org" in res

    def test_get_single_comment_jsonp(self):
        res = self.app.get(
            "/comments/1", {"callback": "callback"}, headers={"accept": "application/javascript"}
        )
    ...
    def test_list_comments_jsonp(self):
        res = self.app.get(
            "/comments/", {"callback": "callback"}, headers={"accept": "application/javascript"}
        )
        assert "application/javascript" in res.headers["Content-Type"]
        assert "callback" in res
    ...
    def test_list_comments_rss(self):
        res = self.app.get("/rss/comments/", headers={"accept": "application/atom+xml"})
    ...
    def test_list_comments_page(self):
        res = self.app.get("/comments/", headers={"accept": "text/html"})
        assert "text/html" in res.headers["Content-Type"]
        assert "libravatar.org" in res
    ...
    def test_list_comments_pagination(self):
        ...
        res = self.app.get("/comments/", {"rows_per_page": 1})
        body = res.json_body
        assert len(body["comments"]) == 1
        comment1 = body["comments"][0]

        res = self.app.get("/comments/", {"rows_per_page": 1, "page": 2})
        body = res.json_body
        assert len(body["comments"]) == 1
    ...
    def test_list_comments_by_nonexistent_update(self):
        res = self.app.get("/comments/", {"updates": "flash-player"}, status=400)
        assert res.json_body["errors"][0]["name"] == "updates"
        assert (
            res.json_body["errors"][0]["description"] == "Invalid updates specified: flash-player"
        )
    ...
    def test_list_comments_by_nonexistent_package(self):
        res = self.app.get("/comments/", {"packages": "flash-player"}, status=400)
        assert res.json_body["errors"][0]["name"] == "packages"
        assert (
            res.json_body["errors"][0]["description"] == "Invalid packages specified: flash-player"
        )
