...
class TestCommentsService(base.BasePyTestCase):
    ...
    def test_get_single_comment_jsonp(self):
        res = self.app.get(
            "/comments/1", {"callback": "callback"}, headers=dict(accept="application/javascript")
        )
    ...
    def test_list_comments_jsonp(self):
        res = self.app.get(
            "/comments/", {"callback": "callback"}, headers=dict(accept="application/javascript")
        )
        assert "application/javascript" in res.headers["Content-Type"]
        assert "callback" in res
    ...
    def test_list_comments_rss(self):
        res = self.app.get("/rss/comments/", headers=dict(accept="application/atom+xml"))
