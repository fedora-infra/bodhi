...
class TestNewUpdate(BasePyTestCase):
    ...
    def test_obsoletion(self, *args):
        ...
        with mock.patch(**mock_uuid4_version1), fml_testing.mock_sends(
            update_schemas.UpdateReadyForTestingV3, update_schemas.UpdateRequestTestingV1
        ):
        ...
        args = self.get_update("bodhi-2.0.0-3.fc17")
        with mock.patch(**mock_uuid4_version2), fml_testing.mock_sends(
            update_schemas.UpdateReadyForTestingV3, update_schemas.UpdateRequestTestingV1
        ):
        ...
        expected_comment = expected_comment.format(
            urlparse.urljoin(
                config["base_address"], f"/updates/FEDORA-{datetime.now().year}-033713b73b"
            )
        )
        ...
        expected_comment = expected_comment.format(
            urlparse.urljoin(
                config["base_address"], f"/updates/FEDORA-{datetime.now().year}-53345602d5"
            )
        )
...
class TestSetRequest(BasePyTestCase):
    ...
    def test_set_request_locked_update(self, *args):
        ...
        post_data = dict(
            update=nvr, request="stable", csrf_token=self.app.get("/csrf").json_body["csrf_token"]
        )
...
class TestUpdatesService(BasePyTestCase):
    ...
    def test_locked_update_links_to_compose_html(self):
        ...
        resp = self.app.get("/updates/%s" % update.alias, headers={"Accept": "text/html"})
    ...
    def test_provenpackager_request_privs(self, *args):
        ...
        with mock.patch("bodhi.server.Session.remove"):
            app = TestApp(main({}, testing="ralph", session=self.db, **self.app_settings))
        post_data = dict(
            update=nvr, request="stable", csrf_token=app.get("/csrf").json_body["csrf_token"]
        )
    ...
    def test_edit_pending_update_with_new_builds(self, info, get_tags, tag_task, *args):
        ...
        info.assert_any_call("Removing %s tag from %s",
                             "f17-updates-testing-pending",
                             "bodhi-2.0.0-2.fc17"
        )
        assert (
            mock.call("Removing %s tag from %s",
...
class TestWaiveTestResults(BasePyTestCase):
    ...
    def test_cannot_waive_test_results_on_locked_update(self, *args):
        ...
        post_data = dict(update=nvr, csrf_token=self.app.get("/csrf").json_body["csrf_token"])
    ...
    def test_waive_test_results_1_of_2_unsatisfied_requirements(
        ...
        post_data = dict(
            update=nvr, tests="atomic_ci_pipeline_results", csrf_token=self.get_csrf_token()
        )
