...
class TestNewUpdate(BasePyTestCase):
    ...
    def test_multiple_builds_of_same_package(self, *args):
        res = self.app.post_json(
            "/updates/", self.get_update(["bodhi-2.0-2.fc17", "bodhi-2.0-3.fc17"]), status=400
        )
        assert "Multiple bodhi builds specified" in res

    def test_invalid_autokarma(self, *args):
        res = self.app.post_json("/updates/", self.get_update(stable_karma=-1), status=400)
        assert "-1 is less than minimum value 1" in res
        res = self.app.post_json("/updates/", self.get_update(unstable_karma=1), status=400)
        assert "1 is greater than maximum value -1" in res

    def test_duplicate_update(self, *args):
        res = self.app.post_json("/updates/", self.get_update("bodhi-2.0-1.fc17"), status=400)
    ...
    def test_no_privs(self, *args):
        ...
        expected_error = {
            ...
            "name": "builds",
            "description": (
                'bodhi is not a member of "packager", which is a mandatory packager group'
            ),
        }
    ...
    def test_invalid_acl_system(self, *args):
        with mock.patch.dict(config, {"acl_system": "null"}):
            res = self.app.post_json("/updates/", self.get_update("bodhi-2.0-2.fc17"), status=403)
    ...
    def test_new_rawhide_rpm_update(self, *args):
        ...
        self.db.commit()
        with fml_testing.mock_sends():
            res = self.app.post_json("/updates/", self.get_update("bodhi-2.0.0-2.fc17"), status=400)
    ...
    def test_new_rpm_update_from_tag(
        ...
        with (
            ...
            mock.patch("bodhi.server.models.Release.mandatory_days_in_testing", 0),
            mock.patch("bodhi.server.models.notifications.publish") as mockpub,
        ):
        ...
        assert msg.body["artifact"]["builds"][0]["nvr"] == "gnome-backgrounds-3.0-1.fc17"
        assert msg.body["artifact"]["builds"][0]["task_id"] == 15051
        assert msg.body["artifact"]["builds"][0]["id"] == 16061
        assert msg.body["artifact"]["type"] == "koji-build-group"
        assert msg.packages == ["gnome-backgrounds"]
    ...
    def test_koji_config_url(self, *args):
        ...
        """
        self.registry.settings["koji_web_url"] = "https://koji.fedoraproject.org/koji/"
        ...
        assert re.search(
            r"https://koji\.fedoraproject\.org/koji/search\?terms=.*\&amp;"
            r"type=build\&amp;match=exact",
            str(resp),
        )
    ...
    def test_koji_config_mock_url_without_trailing_slash(self, *args):
        ...
        assert re.search(
            r"https://host\.org/search\?terms=.*\&amp;type=build\&amp;match=exact", str(resp)
        )
    ...
    def test_multiple_builds_of_same_module_stream(self, *args):
        ...
        res = self.app.post_json(
            "/updates/", self.get_update(["nodejs-6-20170101", "nodejs-6-20170202"]), status=400
        )
    ...
    def test_multiple_builds_of_different_module_stream(self, *args):
        ...
        with fml_testing.mock_sends(api.Message):
            res = self.app.post_json(
                "/updates/", self.get_update(["nodejs-6-20170101", "nodejs-8-20170202"])
            )
        res = res.json
    ...
    def test_obsoletion(self, *args):
        ...
        expected_comment = expected_comment.format(
            urlparse.urljoin(
                config["base_address"],
                f"/updates/FEDORA-{datetime.now(timezone.utc).year}-033713b73b",
            )
        )
        ...
        expected_comment = expected_comment.format(
            urlparse.urljoin(
                config["base_address"],
                f"/updates/FEDORA-{datetime.now(timezone.utc).year}-53345602d5",
            )
        )
    ...
    def test_create_new_nonsecurity_update_when_previous_security_one_exists(self, *args):
        ...
        assert r["caveats"][1]["description"] == (
            "Adjusting type of this update to security,since it obsoletes another security update"
        )
        assert r["request"] == "testing"

        # Since we're obsoleting something owned by someone else.
        assert r["caveats"][2]["description"] == (
            "This update has obsoleted bodhi-2.0.0-2.fc17, and has inherited its bugs and notes."
        )
    ...
    @mock.patch(
        "bodhi.server.services.updates.Update.obsolete_older_updates",
...
class TestSetRequest(BasePyTestCase):
    ...
    def test_test_gating_status_failed(self, info):
        ...
        up.status = UpdateStatus.testing
        up.request = None
        post_data = {"update": nvr, "request": "stable", "csrf_token": self.get_csrf_token()}
        ...
        assert (
            f"Failed to set the request: {config.get('not_yet_tested_msg')}: "
    ...
    @mock.patch(
        "bodhi.server.services.updates.Update.set_request", side_effect=BodhiException("oops!")
    )
    @mock.patch(
        "bodhi.server.services.updates.Update.meets_requirements_why", (True, "a fake reason")
    )
    @mock.patch("bodhi.server.services.updates.log.info")
    def test_BodhiException_exception(self, log_info, send_request, *args):
        ...
        res = self.app.post_json(f"/updates/{post_data['update']}/request", post_data, status=400)
    ...
    @mock.patch(
        "bodhi.server.services.updates.Update.set_request", side_effect=OSError("IOError. oops!")
    )
    @mock.patch(
        "bodhi.server.services.updates.Update.meets_requirements_why", (True, "a fake reason")
    )
...
class TestEditUpdateForm(BasePyTestCase):
    ...
    def test_edit_not_loggedin(self):
        ...
        resp = app.get(
            "/updates/FEDORA-2017-a3bbe1a8f2/edit", status=403, headers={"accept": "text/html"}
        )
    ...
    def test_disabled_unspecified_severity_for_security_update(self):
        ...
        resp = self.app.get(f"/updates/{alias}/edit", headers={"accept": "text/html"})
        assert re.search(
            r'<select id="severity" name="severity">\n.*'
    ...
    def test_days_in_testing_new_update(self):
        ...
        """
        resp = self.app.get("/updates/new", headers={"accept": "text/html"})
        assert re.search(
            r'<input type="number" name="stable_days" placeholder="auto"'
    ...
    def test_xss(self):
        ...
        self.db.commit()
        resp = self.app.get(f"/updates/{update.alias}/edit", headers={"accept": "text/html"})
...
class TestUpdatesService(BasePyTestCase):
    ...
    def test_home_html_legal(self):
        ...
        with mock.patch.dict(self.registry.settings, {"legal_link": "http://loweringthebar.net/"}):
    ...
    def test_edit_invalidly_tagged_build(self):
        ...
        with mock.patch(
            "bodhi.server.buildsys.DevBuildsys.listTags", return_value=[{"name": "f17-updates"}]
        ) as listTags:
            res = self.app.post_json("/updates/", update_json, status=400)
    ...
    def test_edit_koji_error(self):
        ...
        with mock.patch(
            "bodhi.server.buildsys.DevBuildsys.listTags", side_effect=koji.GenericError()
        ) as listTags:
            res = self.app.post_json("/updates/", update_json, status=400)
        ...
        expected_json = {
            ...
            "errors": [
                {
                    "description": "Invalid koji build: bodhi-2.0-1.fc17",
                    "location": "body",
                    "name": "builds",
                }
            ],
    ...
    def test_edit_untagged_build(self):
        ...
        with mock.patch("bodhi.server.buildsys.DevBuildsys.listTags", return_value=[]) as listTags:
            res = self.app.post_json("/updates/", update_json, status=400)
        ...
        expected_json = {
            ...
            "errors": [
                ...
                {
                    "description": (
                        "Cannot find release associated with build: bodhi-2.0-1.fc17, tags: []"
                    ),
                    "location": "body",
                    "name": "builds",
                },
    ...
    def test_get_single_update_rss_fail_406(self):
        ...
        self.app.get(
            f"/updates/{update.alias}", headers={"Accept": "application/atom+xml"}, status=406
        )
    ...
    def test_get_single_update_html_privacy_link(self):
        ...
        with mock.patch.dict(self.registry.settings, {"privacy_link": "https://privacyiscool.com"}):
            resp = self.app.get(f"/updates/{update.alias}", headers={"Accept": "text/html"})
    ...
    def test_list_updates_jsonp(self):
        res = self.app.get(
            "/updates/", {"callback": "callback"}, headers={"Accept": "application/javascript"}
        )
        assert "application/javascript" in res.headers["Content-Type"]
        assert "callback" in res
        assert "bodhi-2.0-1.fc17" in res
    ...
    def test_list_updates_rss(self):
        res = self.app.get("/rss/updates/", headers={"Accept": "application/atom+xml"})
    ...
    def test_list_updates_rss_formatting(self):
        ...
        update = Build.query.filter_by(nvr="bodhi-2.0-1.fc17").one().update
        update_details = (
            f"<h1>FEDORA-{datetime.now(timezone.utc).year}-a3bbe1a8f2</h1>\n"
    ...
    def test_list_updates_rss_with_single_filter(self):
        res = self.app.get(
            "/rss/updates/", {"severity": "low"}, headers={"Accept": "application/atom+xml"}
        )
        assert "application/rss+xml" in res.headers["Content-Type"]
        assert "Released updates" in res
    ...
    def test_list_updates_html(self):
        res = self.app.get("/updates/", headers={"Accept": "text/html"})
    ...
    def test_updates_search(self):
        ...
        update = Build.query.filter_by(nvr="bodhi-2.0-1.fc17").one().update
        res = self.app.get("/updates/", {"search": update.alias})
        body = res.json_body
        assert len(body["updates"]) == 1
        up = body["updates"][0]
        assert up["title"] == "bodhi-2.0-1.fc17"
    ...
    def test_list_updates_pagination(self, *args):
        ...
        res = self.app.get("/updates/", {"rows_per_page": 1})
        body = res.json_body
        assert len(body["updates"]) == 1
        update1 = body["updates"][0]

        res = self.app.get("/updates/", {"rows_per_page": 1, "page": 2})
        body = res.json_body
        assert len(body["updates"]) == 1
    ...
    def test_list_updates_by_approved_since(self):
        ...
        res = self.app.get("/updates/", {"approved_since": now.strftime("%Y-%m-%d")})
        body = res.json_body
        assert not body["updates"]
    ...
    def test_list_updates_by_invalid_approved_since(self):
        res = self.app.get("/updates/", {"approved_since": "forever"}, status=400)
        body = res.json_body
    ...
    def test_list_updates_by_approved_before(self):
        ...
        res = self.app.get("/updates/", {"approved_before": "1984-11-01"})
        body = res.json_body
        assert not body["updates"]
    ...
    def test_list_updates_by_invalid_approved_before(self):
        res = self.app.get("/updates/", {"approved_before": "forever"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_invalid_critpath(self):
        res = self.app.get("/updates/", {"critpath": "lalala"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_invalid_locked(self):
        res = self.app.get("/updates/", {"locked": "maybe"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_modified_since(self):
        ...
        res = self.app.get("/updates/", {"modified_since": now.strftime("%Y-%m-%d")})
        body = res.json_body
        assert not body["updates"]
    ...
    def test_list_updates_by_invalid_modified_since(self):
        res = self.app.get("/updates/", {"modified_since": "the dawn of time"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_modified_before(self):
        ...
        # Try with no modified updates first
        res = self.app.get("/updates/", {"modified_before": now.strftime("%Y-%m-%d")})
        body = res.json_body
        assert not body["updates"]
        ...
        # And try again
        res = self.app.get("/updates/", {"modified_before": tomorrow})
        body = res.json_body
        assert len(body["updates"]) == 1
    ...
    def test_list_updates_by_invalid_modified_before(self):
        res = self.app.get("/updates/", {"modified_before": "the dawn of time"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_invalid_pushed(self):
        res = self.app.get("/updates/", {"pushed": "who knows?"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_pushed_since(self):
        ...
        res = self.app.get("/updates/", {"pushed_since": now.strftime("%Y-%m-%d")})
        body = res.json_body
        assert not body["updates"]
    ...
    def test_list_updates_by_invalid_pushed_since(self):
        res = self.app.get("/updates/", {"pushed_since": "a while ago"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_pushed_before(self):
        ...
        # Try with no pushed updates first
        res = self.app.get("/updates/", {"pushed_before": now.strftime("%Y-%m-%d")})
        body = res.json_body
        assert not body["updates"]
        ...
        # And try again
        res = self.app.get("/updates/", {"pushed_before": tomorrow})
        body = res.json_body
        assert len(body["updates"]) == 1
    ...
    def test_list_updates_by_invalid_pushed_before(self):
        res = self.app.get("/updates/", {"pushed_before": "a while ago"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_list_updates_by_nonexistent_request(self):
        res = self.app.get("/updates/", {"request": "impossible"}, status=400)
        body = res.json_body
        request_vals = ", ".join(UpdateRequest.values())
        assert not body.get("updates")
        assert body["errors"][0]["name"] == "request"
        assert body["errors"][0]["description"] == (f'"impossible" is not one of {request_vals}')
    ...
    def test_list_updates_by_nonexistent_severity(self):
        res = self.app.get("/updates/", {"severity": "schoolmaster"}, status=400)
        body = res.json_body
        severity_vals = ", ".join(UpdateSeverity.values())
        assert not body.get("updates")
        assert body["errors"][0]["name"] == "severity"
        assert body["errors"][0]["description"] == (f'"schoolmaster" is not one of {severity_vals}')
    ...
    def test_list_updates_by_nonexistent_status(self):
        res = self.app.get("/updates/", {"status": "single"}, status=400)
        body = res.json_body
    ...
    def test_list_updates_by_nonexistent_suggest(self):
        res = self.app.get("/updates/", {"suggest": "no idea"}, status=400)
        body = res.json_body
        suggest_vals = ", ".join(UpdateSuggestion.values())
        assert not body.get("updates")
        assert body["errors"][0]["name"] == "suggest"
        assert body["errors"][0]["description"] == (f'"no idea" is not one of {suggest_vals}')
    ...
    def test_list_updates_by_nonexistent_type(self):
        res = self.app.get("/updates/", {"type": "not_my"}, status=400)
        body = res.json_body
    ...
    def test_list_updates_by_nonexistent_username(self):
        res = self.app.get("/updates/", {"user": "santa"}, status=400)
        body = res.json_body
        assert not body.get("updates")
    ...
    def test_edit_update(self, *args):
        ...
        with fml_testing.mock_sends(
            update_schemas.UpdateEditV2, update_schemas.UpdateReadyForTestingV3
        ):
            r = self.app.post_json("/updates/", args)
    ...
    def test_edit_rpm_update_from_tag(self, *args):
        ...
        with (
            mock.patch("bodhi.server.buildsys.DevBuildsys.getTag", self.mock_getTag),
            fml_testing.mock_sends(update_schemas.UpdateReadyForTestingV3),
        ):
            r = self.app.post_json("/updates/", update)
    ...
    def test_edit_pending_update_with_new_builds(self, info, get_tags, tag_task, *args):
        ...
        get_tags.return_value = ["f17-updates-candidate", "f17-updates-testing-pending"]
        ...
        info.assert_any_call("Unpushing %s", "bodhi-2.0.0-2.fc17")
        info.assert_any_call(
            "Removing %s tag from %s", "f17-updates-testing-pending", "bodhi-2.0.0-2.fc17"
        )
        assert (
            mock.call("Removing %s tag from %s", "f17-updates-candidate", "bodhi-2.0.0-2.fc17")
            not in info.call_args_list
        )
        tag_task.delay.assert_called_once_with(
            tag="f17-updates-signing-pending", builds=["bodhi-2.0.0-3.fc17"]
        )
        up = r.json_body
        assert up["title"] == "bodhi-2.0.0-3.fc17"
        assert up["status"] == "pending"
    ...
    @mock.patch("bodhi.server.models.Build.get_tags")
    @mock.patch("bodhi.server.models.log.info")
    def test_edit_testing_update_with_new_builds_with_stable_request(
        self, info, get_tags, tag_task, *args
    ):
        nvr = "bodhi-2.0.0-2.fc17"
        args = self.get_update(nvr)
        ...
        get_tags.return_value = ["f17-updates-testing", "f17-updates-pending"]
        ...
        info.assert_any_call("Unpushing %s", "bodhi-2.0.0-2.fc17")
        info.assert_any_call("Removing %s tag from %s", "f17-updates-pending", "bodhi-2.0.0-2.fc17")
        info.assert_any_call(
            "Moving %s from %s to %s",
            "bodhi-2.0.0-2.fc17",
            "f17-updates-testing",
    ...
    def test_edit_pending_sidetag_update_with_new_builds(self, info, get_tags, tag_task, *args):
        ...
        info.assert_any_call(
            "Removing %s tag from %s", "f17-updates-candidate", "bodhi-2.0.0-2.fc17"
        )
        tag_task.delay.assert_any_call(
            tag="f17-updates-signing-pending", builds=["bodhi-2.0.0-3.fc17"]
        )
        tag_task.delay.assert_any_call(tag="f17-updates-candidate", builds=["bodhi-2.0.0-3.fc17"])
        up = r.json_body
        assert up["title"] == "bodhi-2.0.0-3.fc17"
        assert up["status"] == "pending"
        assert up["request"] == "testing"
    ...
    def test_edit_testing_sidetag_update_with_new_builds(self, info, get_tags, tag_task, *args):
        ...
        get_tags.return_value = ["f17-updates-testing", "f17-build-side-12345"]
        ...
        info.assert_any_call("Unpushing %s", "bodhi-2.0.0-2.fc17")
        info.assert_any_call("Removing %s tag from %s", "f17-updates-testing", "bodhi-2.0.0-2.fc17")
        tag_task.delay.assert_any_call(
            tag="f17-updates-signing-pending", builds=["bodhi-2.0.0-3.fc17"]
        )
        up = r.json_body
    ...
    def test_edit_locked_update(self, *args):
        ...
        assert "errors" in r
        assert {
            "description": "Can't add builds to a locked update",
            "location": "body",
            "name": "builds",
        } in r["errors"]
    ...
    def test_invalid_request(self, *args):
        ...
        resp = self.app.post_json(
            f"/updates/{args['builds']}/request",
            {"request": "foo", "csrf_token": self.get_csrf_token()},
            status=400,
        )
        resp = resp.json_body
        request_vals = ", ".join(UpdateRequest.values())
        ...
        resp = self.app.post_json(
            f"/updates/{args['builds']}/request",
            {"request": None, "csrf_token": self.get_csrf_token()},
            status=400,
        )
        resp = resp.json_body
        assert resp["status"] == "error"
    ...
    def test_request_after_unpush(self, *args):
        ...
        with fml_testing.mock_sends(
            update_schemas.UpdateCommentV1, update_schemas.UpdateRequestUnpushV1
        ):
            resp = self.app.post_json(
                f"/updates/{up.alias}/request",
    ...
    def test_edit_missing_update(self, *args):
        ...
        r = self.app.post_json("/updates/", args, status=400).json_body
        assert r["status"] == "error"
        assert r["errors"][0]["description"] == f"Cannot find update to edit: {edited}"
    ...
    def test_autopush_reached_main(self, autokarma, cbb, critpath, status):
        ...
        if cbb and autokarma:
            ...
            assert msgs == [
                update_schemas.UpdateRequestStableV1.from_dict({"update": up, "agent": "bodhi"}),
                update_schemas.UpdateKarmaThresholdV1.from_dict({"update": up, "status": "stable"}),
                ...
                [
                    {
                        "name": "karma",
                        "description": (
                            "This update has not yet met the minimum testing requirements "
    ...
    def test_update_severity_label_present_correctly_when_severity_is_urgent(self, *args):
        ...
        self.assert_severity_html(
            UpdateSeverity.urgent, ['<div class="col fw-bold text-muted">Severity', "urgent"]
        )
    ...
    def test_update_severity_label_present_correctly_when_severity_is_high(self, *args):
        ...
        """
        self.assert_severity_html(
            UpdateSeverity.high, ['<div class="col fw-bold text-muted">Severity', "high"]
        )
    ...
    def test_update_severity_label_present_correctly_when_severity_is_medium(self, *args):
        ...
        """
        self.assert_severity_html(
            UpdateSeverity.medium, ['<div class="col fw-bold text-muted">Severity', "medium"]
        )
    ...
    def test_update_severity_label_present_correctly_when_severity_is_low(self, *args):
        ...
        """
        self.assert_severity_html(
            UpdateSeverity.low, ['<div class="col fw-bold text-muted">Severity', "low"]
        )
    ...
    def test_edit_update_with_expired_override(self, *args):
        """
        Edit the update with expired override
        """
        user = User(name="bob")
        self.db.add(user)
        self.db.commit()
    ...
    def test_edit_testing_update_with_build_from_unpushed_update(self, *args):
        ...
        assert unpushed_update.title == unpushed_alias
    ...
    def test_frozen_release_html(self, update_status, release_state):
        ...
        if update_status != UpdateStatus.stable and release_state == ReleaseState.frozen:
            assert (
                "This update will not be pushed to stable until freeze is lifted "
                f"from {release.long_name}."
            ) in resp
        else:
            assert (
                "This update will not be pushed to stable until freeze is lifted "
    ...
    def test_koji_build_url_encoding(self):
        ...
        assert "text/html" in resp.headers["Content-Type"]
        assert (
            'search?terms=hylafax%2B-7.0.3-1.fc17&amp;type=build&amp;match=exact" '
            'target="_blank">hylafax+-7.0.3-1.fc17</a>'
        ) in resp
...
class TestWaiveTestResults(BasePyTestCase):
    ...
    def test_cannot_waive_test_results_on_locked_update(self, *args):
        ...
        post_data = {"update": nvr, "csrf_token": self.app.get("/csrf").json_body["csrf_token"]}
    ...
    @mock.patch(
        "bodhi.server.services.updates.Update.waive_test_results",
    ...
    def test_BodhiException_exception(self, log_error, waive_test_results, *args):
        ...
        res = self.app.post_json(
            f"/updates/{post_data['update']}/waive-test-results", post_data, status=400
        )
    ...
    def test_waive_test_results_2_unsatisfied_requirements(
        ...
        post_data = {"update": nvr, "csrf_token": self.get_csrf_token()}
    ...
    @mock.patch("bodhi.server.models.User.openid", mock.MagicMock(return_value=None))
    @mock.patch("bodhi.server.models.User.avatar", mock.MagicMock(return_value=None))
    def test_waive_test_results_unfailing_tests(self, greenwave_api_post, waiverdb_api_post, *args):
        """Ensure that waiverdb and greenwaved are properly called when greenwave returns only two
...
class TestGetTestResults(BasePyTestCase):
    ...
    @mock.patch(
        "bodhi.server.services.updates.Update.get_test_gating_info",
    ...
    def test_get_test_results_erroring_on_greenwave(self, call_api, *args):
        ...
        update = Build.query.filter_by(nvr="bodhi-2.0-1.fc17").one().update
        update.locked = False
        call_api.side_effect = requests.exceptions.HTTPError("Un-expected error foo bar")
    ...
    def test_get_test_results_timing_out_on_greenwave(self, call_api, *args):
        ...
        """
        update = Build.query.filter_by(nvr="bodhi-2.0-1.fc17").one().update
        call_api.side_effect = requests.exceptions.Timeout("Request to greenwave timed out")
    ...
    def test_get_test_results_calling_greenwave_unauth(self, call_api, *args):
        ...
        # with mock.patch('bodhi.server.Session.remove'):
        app = TestApp(main({}, session=self.db, **anonymous_settings))
        res = app.get(f"/updates/{nvr}/get-test-results", status=404)
...
class TestTriggerTests(BasePyTestCase):
    ...
    def test_update_status_not_testing(self, *args):
        ...
        post_data = {"csrf_token": self.get_csrf_token()}
