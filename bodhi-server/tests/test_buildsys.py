...
class TestGetKrbConf:
    ...
    def test_krb_ccache_uid(self):
        ...
        assert config == {"ccache": f"a_ccache_{os.geteuid()}"}
...
class TestKojiLogin:
    ...
    def test_AuthError(self, sleep, error, gssapi_login):
        ...
        for key, value in default_koji_opts.items():
            assert value == client.opts[key]
    ...
    def test_login_failure(self, error, gssapi_login):
        ...
        assert type(client) is koji.ClientSession
        gssapi_login.assert_called_once_with(
...
class TestSetupBuildsystem:
    ...
    def test_authenticate_false(self, gssapi_login):
        ...
        config = {
            ...
            "krb_principal": "a_principal",
            "buildsystem": "koji",
            "koji_hub": "https://example.com/koji",
        }
    ...
    def test_nonsense_buildsystem(self):
        ...
        assert buildsys._buildsystem is None
        pytest.raises(
            ValueError, buildsys.setup_buildsystem, {"buildsystem": "Something unsupported"}
        )
