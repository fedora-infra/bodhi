...
class TestAddUpdate(UpdateInfoMetadataTestCase):
    ...
    def test_build_not_in_builds(self):
        ...
        now = datetime(year=2018, month=2, day=8, hour=12, minute=41, second=4, tzinfo=timezone.utc)
        # dates in bodhi db are naive
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                datetime(
                    year=2022, month=11, day=23, hour=12, minute=41, second=4, tzinfo=timezone.utc
                ),
                datetime(
                    year=2022, month=11, day=23, hour=11, minute=41, second=4, tzinfo=timezone.utc
                ),
...
class TestUpdateInfoMetadata(UpdateInfoMetadataTestCase):
    ...
    def test___init___closes_shelve_on_error(self, shelve_open):
        ...
        with (
            mock.patch.object(
                UpdateInfoMetadata, "_fetch_updates", side_effect=OSError("Koji is having a day.")
            ),
...
class TestInsertUpdateinfo(UpdateInfoMetadataTestCase):
    ...
    def setup_method(self, method):
        ...
        self.mkstemp = mock.patch(
            "bodhi.server.metadata.tempfile.mkstemp", side_effect=recording_mkstemp
        )
    ...
    def test_temporary_file_removed_when_modifyrepo_raises(self):
        ...
        with (
            self.mkstemp,
            mock.patch("bodhi.server.metadata.modifyrepo", side_effect=OSError("No repo for you.")),
            pytest.raises(OSError),
        ):
    ...
    def test_no_descriptor_leaked_when_xml_dump_raises(self):
        ...
        self.md.uinfo.xml_dump.side_effect = OSError("No updateinfo for you.")
        fds_before = len(os.listdir("/proc/self/fd"))

        with self.mkstemp, pytest.raises(OSError):
            self.md.insert_updateinfo(self.tempcompdir)
