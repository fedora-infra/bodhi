...
class TestAvatar:
    ...
    def test_libravatar_dns_set_ssl_false(self, libravatar_url):
        ...
        libravatar_url.assert_called_once_with(
            email="bowlofeggs@bodhi-dev.example.com", https=False, size=50, default="retro"
        )
    ...
    def test_libravatar_dns_set_ssl_true(self, libravatar_url):
        ...
        assert avatar == "cool url"
        libravatar_url.assert_called_once_with(
            email="bowlofeggs@bodhi-dev.example.com", https=True, size=50, default="retro"
        )
    ...
    def test_libravatar_user_without_email(self, libravatar_url):
        ...
        assert avatar == "cool url"
        libravatar_url.assert_called_once_with(
            email="default", https=True, size=50, default="retro"
        )
...
class TestCanWaiveTestResults(base.BasePyTestCase):
    ...
    def test_access_token_undefined(self):
        ...
        config.update({"test_gating.required": True, "waiverdb.access_token": None})
    ...
    def test_can_waive_test_results(self):
        config.update({"test_gating.required": True, "waiverdb.access_token": "secret"})
        u = Update.query.all()[0]
        u.test_gating_status = TestGatingStatus.failed
        u.status = models.UpdateStatus.testing
    ...
    def test_gating_required_false(self):
        ...
        """
        config.update({"test_gating.required": False, "waiverdb.access_token": "secret"})
        u = Update.query.all()[0]
        u.test_gating_status = TestGatingStatus.failed
        u.status = models.UpdateStatus.testing
        assert not util.can_waive_test_results(None, u)
...
class TestSanityCheckRepodata(base.BasePyTestCase):
    ...
    def test_invalid_repo_exception(self, *args):
        ...
        pytest.importorskip(
            "libdnf5", reason="This tests correct behavior with libdnf5 which is not installed"
        )
        base.mkmetadatadir(self.tempdir, compress_type="xz")
    ...
    @pytest.mark.skipif(
        "libdnf5" in sys.modules, reason="This can only be tested if lidnf5 is not installed"
    )
...
class TestUtils(base.BasePyTestCase):
    def test_get_critpath_components_dummy(self):
        ...
        list.
        """
        config.update({"critpath.type": None, "critpath_pkgs": ["kernel", "glibc"]})
    ...
    def test_get_critpath_components_not_json_not_rpm(self, mock_log):
        ...
        mock_log.warning.assert_called_once_with(
            'The critpath.type of "%s" does not support searching for non-RPM components',
            "(default)",
        )
    ...
    def test_get_grouped_critpath_components_success(self, critpath_json_config):
        ...
        filtered = util.get_grouped_critpath_components("f36", components=["ModemManager-glib"])
    ...
    def test_get_ccp_components_json_bad_file(self, mock_log, ccp_json_config):
        ...
        config.update({"critpath.jsonpath": tempdir})
    ...
    def test_get_ccp_components_success(self, ccp_json_config):
        ...
        assert grouped == testdata["rpm"]
        # now test with components arg
        filtered = util.get_ccp_components("f36", components=["ModemManager"])
        assert filtered == {
            "Everything": {
                "aarch64": {
                    "image": [
    ...
    def test_pagure_api_get(self, session):
        ...
        expected_json = {
            ...
            "access_users": {"admin": [], "commit": [], "owner": ["mprahl"], "ticket": []},
            "close_status": [],
            "custom_keys": [],
    ...
    def test_greenwave_api_post(self, session):
        ...
        decision = util.greenwave_api_post("http://domain.local/api/v1.0/decision", data)
    ...
    def test_greenwave_api_post_500_error(self, sleep, session):
        ...
        with pytest.raises(RuntimeError) as exc:
            ...
            util.greenwave_api_post("http://domain.local/api/v1.0/decision", data)
    ...
    def test_greenwave_api_post_non_500_error(self, sleep, session):
        ...
        session.post.return_value.json.return_value = {"message": "Not found."}
        with pytest.raises(RuntimeError) as exc:
            data = {
                "product_version": "fedora-26",
                "decision_context": "bodhi_push_update_stable",
    ...
    def test_waiverdb_api_post(self, session):
        ...
        waiver = util.waiverdb_api_post("http://domain.local/api/v1.0/waivers/", data)
    ...
    def test_markup_with_bleach(self, clean):
        ...
        clean.assert_called_once_with(
            expected_text, tags=expected_tags, attributes=expected_attributes
        )
    ...
    def test_cmd_closes_pipes_when_communicate_raises(self):
        ...
        with (
            mock.patch("bodhi.server.util.subprocess.Popen", RecordingPopen),
...
class TestCMDFunctions:
    ...
    def test_err_nonzero_return_code(self, mock_popen, mock_error, mock_debug):
        ...
        assert mock_error.mock_calls == [
            mock.call("/bin/echo returned a non-0 exit code: 1"),
            mock.call("output\nerror"),
        ]
    ...
    def test_no_err_zero_return_code(self, mock_popen, mock_error, mock_debug):
        ...
        assert mock_debug.mock_calls == [
            mock.call("Running /bin/echo"),
            mock.call("subprocess output: output\nNone"),
        ]
    ...
    def test__get_build_repository_and_digest(self, session):
        ...
        session.return_value.getBuild.return_value = {
            "extra": {"typeinfo": {"image": {"index": {"pull": pull_specs}}}}
        }
...
@mock.patch(
    "bodhi.server.util._container_image_url",
    new=lambda sr, r, tag=None, digest=None: f"{sr}:{r}:{tag}" if tag else f"{sr}:{r}@{digest}",
)
@mock.patch(
    "bodhi.server.util._get_build_repository_and_digest",
...
class TestCopyContainer(base.BasePyTestCase):
    ...
    def test_with_destination_registry(self, cmd):
        ...
        cmd.assert_called_once_with(
            ["skopeo", "copy", "src:testrepo@sha256:f00d00", "boo:testrepo:1-1"],
            raise_on_error=True,
        )
...
class TestPageUrl:
    ...
    def test_multi_values_parameter(self):
        ...
        context.get().params = MultiDict(
            [("search", ""), ("status", "pending"), ("status", "testing")]
        )
        page = 2
