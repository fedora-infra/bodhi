...
class TestOIDCLoginViews(base.BasePyTestCase):
    ...
    def test_login(self):
        ...
        assert set(query_data["scope"].split(" ")) == {
            "openid",
            "email",
            "profile",
    ...
    def test_authorize(self):
        ...
        request = testing.DummyRequest(
            path="/oidc/authorize", params={"state": "STATE", "code": "CODE"}
        )
    ...
    def test_authorize_error(self):
        ...
        with (
            mock.patch("requests.sessions.Session.send", side_effect=fake_send()),
            pytest.raises(HTTPUnauthorized) as exc,
        ):
    ...
    def test_login_with_token(self):
        ...
        request = testing.DummyRequest(
            path="/oidc/login-with-token", headers={"Authorization": "Bearer TOKEN"}
        )
        request.registry = self.registry
        request.db = self.db
