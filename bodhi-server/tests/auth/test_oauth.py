...
class TestOAuth1(base.BasePyTestCase):
    ...
    def test_oauth1_authorize(self):
        ...
        request2 = testing.DummyRequest(
            path=parsed_url.path, params=dict(urlparse.parse_qsl(parsed_url.query))
        )
    ...
    @pytest.mark.skipif(
        parse_version(authlib_version) < parse_version("1.0.0"), reason="Only on Authlib >= 1.0"
    )
    def test_oauth1_authorize_no_state(self):
        ...
        request = testing.DummyRequest(path="/oauth1", params={})
    ...
    def test_oauth1_authorize_no_data(self):
        ...
        request = testing.DummyRequest(path="/oauth1", params={"oauth_token": "foo", "state": "S"})
        with pytest.raises(OAuthError) as exc:
            client.authorize_access_token(request)
...
class TestOAuth2(base.BasePyTestCase):
    ...
    def test_oauth2_authorize(self):
        ...
        with mock.patch("requests.sessions.Session.send") as send:
            ...
            request2 = testing.DummyRequest(
                path="/authorize", params={"state": state, "code": "dummycode"}
            )
            request2.session = request.session
    ...
    def test_update_token(self):
        ...
        old_token = dict(access_token="a", refresh_token="b", token_type="bearer", expires_at=100)
