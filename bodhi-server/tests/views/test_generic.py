...
class TestExceptionView(base.BasePyTestCase):
    ...
    @mock.patch(
        "bodhi.server.views.generic._generate_home_page_stats",
...
class TestGenericViews(base.BasePyTestCase):
    ...
    def test_markdown_with_unknown_prefixed_bugzilla(self):
        ...
        assert res.json_body["html"] == (
            '<div class="markdown"><p>Crazy.  upstream#12345 is still busted.</p></div>'
        )
    ...
    def test_markdown_with_unknown_prefix_known_substring(self):
        ...
        assert res.json_body["html"] == (
            '<div class="markdown"><p>Crazy.  aRHBZa#12345 is still busted.</p></div>'
        )
    ...
    def test_markdown_with_autolink_without_http(self):
        ...
        assert res.json_body["html"] == (
            '<div class="markdown"><p><a href="http://getfedora.org">getfedora.org</a></p></div>'
        )
...
class TestCritpathEndpoint(base.BasePyTestCase):
    ...
    def test_component_list(self, critpath_json_config):
        ...
        (tempdir, _testdata) = critpath_json_config
        config.update({"critpath.type": "json", "critpath.jsonpath": tempdir})
        res = self.app.get(
            "/get_critpath_components",
            {"collection": "f36", "components": "ModemManager-glib,NetworkManager"},
        )
        body = res.json_body
...
class TestCcpEndpoint(base.BasePyTestCase):
    ...
    def test_component_list(self, ccp_json_config):
        """Components parameter is a comma separated values list."""
        (tempdir, _testdata) = ccp_json_config
        config.update({"critpath.type": "json", "critpath.jsonpath": tempdir})
        res = self.app.get(
            "/get_ccp_components",
            {"collection": "f36", "components": "acl,attr,ModemManager,accountsservice"},
        )
        body = res.json_body
        assert body == {
            "Everything": {
