...
def _expected_skopeo_args(config, source, destination, dtag, extra_args=None):
...
def _expected_skopeo_call(config, source, destination, dtag, extra_args=None):
    if extra_args is None:
        extra_args = []
    return mock.call(
...
<revision>1508375628</revision></repomd>"""
            ...
            compose_dir = os.path.join(
                ...
                (
                    f"{release.id_prefix.title()}-{int(release.version)}-"
                    f"{reqtype}-{d.year}{d.month:02d}{d.day:02d}.0"
                ),
            )
...
That was the actual one"""
    ...
    def test___init___missing_paths(self, mock_os_path_exists):
        ...
        def os_path_exists_se(value):
            return value != "/does/really/not/exist"
        ...
        for s in ("pungi.cmd", "compose_dir", "compose_stage_dir"):
            with (
                mock.patch.dict(config, {s: "/does/really/not/exist"}),
...
"""
        ...
        assert body.startswith(
            f"From: updates@fedoraproject.org\r\nTo: {config.get('fedora_test_announce_list')}"
            ...
            "=========================================\n libseccomp-2.1.0-1.fc20 (FEDORA-"
            f"{time.strftime('%Y')}-a3bbe1a8f2)\n Enhanced "
            "seccomp library\n---------------------------------------------------------------"
            "-----------------\nUpdate Information:\n\nUseful details!\n---------------------"
            "-----------------------------------------------------------\nChangeLog:\n\n* Sat "
...
That was the actual one"""
    ...
    def test_compose_early_exit(self, *args):
        ...
        with (
            mock.patch("bodhi.server.tasks.composer.time.sleep", lambda x: real_sleep(0.125)),
            mock_sends(*[base_schemas.BodhiMessage] * 3),
        ):
            t.run()
    ...
    def test_compose_late_exit(self, *args):
        ...
        with self.db_factory() as session:
            with tempfile.NamedTemporaryFile(delete=False) as script:
                ...
                with (
                    mock.patch.dict(config, {"pungi.cmd": script.name}),
                    mock_sends(*[base_schemas.BodhiMessage] * 3),
                ):
                    t.run()
    ...
    def test_clean_old_composes_param(self, sleep, wfs, wfrs, stage, scr, clean_old):
        ...
        actual_dirs = {
            ...
            if os.path.isdir(os.path.join(compose_dir, d)) and not d.startswith("Fedora-17-updates")
        }
    ...
    def test_compose_module(self, *args):
        ...
        with (
            self.db_factory() as session,
            mock.patch("bodhi.server.tasks.composer.subprocess.Popen") as Popen,
        ):
...
"""
    ...
    def test_modify_stable_bugs(self, close, comment, *args):
        ...
        close.assert_called_with(
            12345,
            versions={"bodhi": "bodhi-2.0-1.fc17"},
            comment=(
...
class TestPungiComposerThread__compose_updates(ComposerThreadBaseTestCase):
    ...
    def test_pungi_cleaned_up_on_exception(self):
        ...
        with (
            ...
            pytest.raises(Exception, match="koji is sad"),
        ):
            t._compose_updates()
...
class TestComposerThread_send_notifications(ComposerThreadBaseTestCase):
    ...
    def test_getlogin_raising_oserror(self):
        ...
        with (
            mock.patch("bodhi.server.tasks.composer.os.getlogin", side_effect=OSError()),
            mock_sends(*expected_messages),
        ):
...
class TestPungiComposerThread__stage_repo(ComposerThreadBaseTestCase):
    ...
    def test_old_link_present(self, mocked_log):
        ...
        assert mocked_log.info.mock_calls == [mock.call("Creating symlink: %s => %s", link, t.path)]
