...
class TestUsersService(base.BasePyTestCase):
    ...
    def test_get_single_user_page(self):
        res = self.app.get("/users/guest", headers={"accept": "text/html"})
    ...
    def test_get_single_user_jsonp(self):
        res = self.app.get(
            "/users/guest", {"callback": "callback"}, headers={"accept": "application/javascript"}
        )
        assert "application/javascript" in res.headers["Content-Type"]
        assert "libravatar.org" in res

    def test_get_single_user_rss(self):
        self.app.get("/users/bodhi", headers={"accept": "application/atom+xml"}, status=406)
    ...
    def test_list_users_jsonp(self):
        res = self.app.get(
            "/users/", {"callback": "callback"}, headers={"accept": "application/javascript"}
        )
        assert "application/javascript" in res.headers["Content-Type"]
    ...
    def test_list_users_rss(self):
        res = self.app.get("/rss/users/", headers={"accept": "application/atom+xml"})
