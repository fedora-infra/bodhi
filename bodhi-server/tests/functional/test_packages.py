...
class TestRpmPackagesService(base.BasePyTestCase):
    ...
    def test_filter_by_name(self):
        ...
        self.db.add(RpmPackage(name="a_second_package"))
        self.db.commit()
        resp = self.app.get("/packages/", {"name": "bodhi"})
        body = resp.json_body
    ...
    def test_filter_by_like(self):
        ...
        self.db.add(RpmPackage(name="a_second_package"))
        self.db.commit()
        resp = self.app.get("/packages/", {"like": "odh"})
        body = resp.json_body
        assert len(body["packages"]) == 1
    ...
    def test_filter_by_search(self):
        ...
        # test search
        resp = self.app.get("/packages/", {"search": "bodh"})
        body = resp.json_body
        assert len(body["packages"]) == 1
        ...
        resp = self.app.get("/packages/", {"search": "corebird"})
        body = resp.json_body
