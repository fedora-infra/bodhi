...
class TestReleasesService(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        release = Release(
            ...
            branch=u'f22',
            critpath_pkgs=[Package.get(u'bodhi')])
    ...
    def test_add_critpath_packages_not_in_bodhi(self, client_session):
        ...
        koji.listPackages.return_value = {'package_name': 'not_in_bodhi'}

        self.app.post('/releases/F22/critpath', {'packages': 'python,not_in_bodhi'})

        release = self.db.query(Release).filter(Release.name == 'F22').one()
        pkg_list = {pkg.name for pkg in release.critpath_pkgs}

        self.assertEqual(pkg_list, {'python', 'not_in_bodhi'})

    def test_add_critpath_packages_invalid_name(self):
        res = self.app.post(
            '/releases/F22/critpath', {'packages': 'not_package'}, status=400
        )
