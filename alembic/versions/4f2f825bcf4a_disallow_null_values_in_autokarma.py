...
def upgrade():
    """
    Find all records with autokarma set to NULL and change them to be False, then disallow further NULL values.
    """
    ...
    op.execute(updates.update().where(updates.c.autokarma == None).values({'autokarma': False}))
...
def downgrade():
    """
    Re-allow NULL values. We can't know which False's used to be NULL because that information is lost.
    """
